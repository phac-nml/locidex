...
class search_db_conf:
    ...
    def __init__(self, input_dir,db_basenames,config_required_fields,):
        ...
        for label in db_basenames:
            if label in ['config','meta']:
                continue
            p = self.get_file_path_by_name(db_basenames[label])
            if p != '':
                self.blast_paths[label] = p

        if len(self.blast_paths):
